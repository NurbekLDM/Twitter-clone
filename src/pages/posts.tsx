...
const PostsPage = () => {
  ...
  const isUserFollowed = (userId) => {
    return userFollowed.some(followedUser => followedUser.following_id === userId);
  };

  useEffect(() => {
    const timer = setTimeout(() => {
      const getUserFollowedUsers = async () => {
        try {
          const response = await authService.getUserFollowed();
          console.log("User Followed users:", response.data);
  ...
  const handleFollow = async (userId) => {
    try {
      ...
      // Refresh followed users after following
      const followedResponse = await authService.getUserFollowed();
      setUserFollowed(followedResponse.data);
  ...
  const handleUnfollow = async (userId) => {
    try {
      const response = await authService.unfollowUser({ followingId: userId });
      console.log("Unfollow user response:", response);

      // Refresh followed users after unfollowing
      const followedResponse = await authService.getUserFollowed();
      setUserFollowed(followedResponse.data);
                  ...
                  {users.map((account, index) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <Button
                          variant={isUserFollowed(account.id) ? "secondary" : "default"}
                          size="sm"
                          className={`rounded-full transition-all duration-300 ${
                            isUserFollowed(account.id)
