...
// Define Post interface based on usage in the code
interface Post {
  id: string;
  text: string;
  image?: string;
...
// Define Comment interface based on usage in the code
interface Comment {
  id: string;
  text: string;
  date: string;
...
const Posts: React.FC = () => {
  const [posts, setPosts] = useState<Post[]>([]); // Replaced any[] with Post[]
  const [loading, setLoading] = useState(true);
  const [likedPosts, setLikedPosts] = useState<{ [key: string]: boolean }>({});
  const [bookmarkedPosts, setBookmarkedPosts] = useState<{ [key: string]: boolean }>({});
  const [comments, setComments] = useState<{ [key: string]: Comment[] }>({}); // Adjusted to Comment[]
  ...
  const openComments = async (postId: string) => {
    ...
    try {
      ...
      if (commentsResponse?.data) { // Assuming commentsResponse has a data field
        setComments((prev) => ({
          ...prev,
          [postId]: commentsResponse.data as Comment[],
        }));
      }
            ...
            <div className="p-4">
              {comments[activePostId!]?.length === 0 ? (
                ...
                <div className="space-y-4">
                  {comments[activePostId!]?.map((comment: Comment) => ( // Replaced any with Comment
                    <article
                      key={comment.id}
